...
if (isset($_GET['d_id'])) {
    ...
    if ($del_sql) {
        message("edit_product.php", "Deleted Successfully!");
    }
}
//edit the product
if (isset($_POST['edit'])) {
...
if (isset($_POST["search"])) {
    ...
    if (mysqli_num_rows($search_sql) > 0) {
        while ($row13 = mysqli_fetch_assoc($search_sql)) {
            $outputs .= '
                        <div class="col-md-3 p-3 space border rounded border-dark">
                            <h4>' . $row13["p_name"] . '</h4>
                            <img class="rounded item-img" src="images/' . $row13["p_category"] . '/' . $row13["p_img"] . '" alt="">
            ...
            if (isset($_SESSION["shopping_cart"])) {
                ...
                if (!in_array($row13["p_id"], $item_array_id)) {
                    $outputs .= '<button onclick="addToCarts(' . $row13["p_id"] . ')" class="btn btn-outline-dark">Add to cart</button>';
                } else {
...
if (isset($_POST["display"])) {
    $output = '<table class="table table-bordered table-hover text-center">
        ...
        foreach ($_SESSION["shopping_cart"] as $keys => $values) {
            $output .= '
                            <tr>
                                ...
                                <td><i class="fa fa-plus-square text-danger btn" onclick="plus(' . $values["item_id"] . ')"></i> <span id=' . $values["item_id"] . '>' . $values["item_quantity"] . '</span> <i onclick="minus(' . $values["item_id"] . ')" class="fa fa-minus-square text-danger btn"></i></a> ' . $values["item_measure"] . ' </td>
                                <td>₹ ' . $values["item_price"] . '</td>
                                <td>₹ ' . number_format($values["item_quantity"] * $values["item_price"], 2) . '</td>
                                <td><i class="fa fa-trash fa-lg text-danger btn" onclick="removeItem(' . $values["item_id"] . ')"></i></td>
                            </tr>';
            $total = $total + ($values["item_quantity"] * $values["item_price"]);
        }
...
if (isset($_POST["display_item"])) {
    // <!--vegetables-->
    $output = '<div class="container-fluid bg-info text-center">
                   <h1 id="Vegetables" class="pt-2">Vegetables</h1>
                   <div class="row justify-content-center">';
    $vegu = mysqli_query($conn, "SELECT * FROM product WHERE p_category = 'Vegetables'");
    while ($row5 = mysqli_fetch_assoc($vegu)) {
        $output .= '
                                <div class="col-md-3 p-3 space border rounded border-white">
                                    <h4>' . $row5["p_name"] . '</h4>
                                    <img class="rounded item-img" src="images/Vegetables/' . $row5["p_img"] . '" alt="">
        ...
        if (isset($_SESSION["shopping_cart"])) {
            ...
            if (!in_array($row5["p_id"], $item_array_id)) {
                $output .= '<button onclick="addToCart(' . $row5["p_id"] . ')" class="btn btn-outline-light">Add to cart</button>';
            } else {
                $output .= '<button class="btn btn-success disabled">Added to Cart</button>';
            }
        } else {
            $output .= '<button onclick="addToCart(' . $row5["p_id"] . ')" class="btn btn-outline-light">Add to cart</button>';
        }
    ...
    $output .= '<div class="container-fluid bg-success text-center">
    ...
    while ($row6 = mysqli_fetch_assoc($drinku)) {
        $output .= '
                                   <div class="col-md-3 space p-3 border rounded border-white">
                                       <h4>' . $row6["p_name"] . '</h4>
                                       <img class="rounded item-img" src="images/Drinks/' . $row6["p_img"] . '" alt="">
        ...
        if (isset($_SESSION["shopping_cart"])) {
            ...
            if (!in_array($row6["p_id"], $item_array_id)) {
                $output .= '<button onclick="addToCart(' . $row6["p_id"] . ')" class="btn btn-outline-light">Add to cart</button>';
            } else {
                $output .= '<button class="btn btn-success disabled">Added to Cart</button>';
            }
        } else {
    ...
    $output .= '<div class="container-fluid bg-danger text-center">
    ...
    while ($row7 = mysqli_fetch_assoc($fruitu)) {
        $output .= '
                                   <div class="col-md-3 space p-3 border rounded border-white">
                                       <img class="rounded item-img" src="images/Fruits/' . $row7["p_img"] . '" alt="">
                                       <h4>' . $row7["p_name"] . '</h4>
        ...
        if (isset($_SESSION["shopping_cart"])) {
            ...
            if (!in_array($row7["p_id"], $item_array_id)) {
                $output .= '<button onclick="addToCart(' . $row7["p_id"] . ')" class="btn btn-outline-light">Add to cart</button>';
            } else {
                $output .= '<button class="btn btn-success disabled">Added to Cart</button>';
            }
        } else {
    ...
    $output .= '<div class="container-fluid bg-primary text-center">
                   <h1 id="Oils" class="text-center">Oils</h1>
                   <div class="row justify-content-center">';
    ...
    while ($row8 = mysqli_fetch_assoc($oilu)) {
        $output .= '
                               <div class="col-md-3 space p-3 border rounded border-white">
                                <h4>' . $row8["p_name"] . '</h4>
                                   <img class="rounded item-img" src="images/Oils/' . $row8["p_img"] . '" alt="">
        ...
        if (isset($_SESSION["shopping_cart"])) {
            ...
            if (!in_array($row8["p_id"], $item_array_id)) {
                $output .= '<button onclick="addToCart(' . $row8["p_id"] . ')" class="btn btn-outline-light">Add to cart</button>';
            } else {
                $output .= '<button class="btn btn-success disabled">Added to Cart</button>';
            }
        } else {
